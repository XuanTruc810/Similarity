...
class similarity_validation:
    ...
    def __init__(self, data, active_col, query, scores = 'tanimoto',
    ...
    def plot_roc(self,fpr, tpr, auc, model, base_name):
    ...
    def plot_ap(self,precision, recall, ap, model, base_name):
    ...
    def validation(self):
        ...
        metric =[self.model,self.list_ap, self.list_roc_auc, self.list_log_roc_auc, 
                 self.list_ef1, self.list_ef5, self.list_ef10, 
                 self.list_f1,self.list_GH] 
    ...
    def visualize(self):
        if self.plot_type == 'roc':
            ...
            for i in self.simi_col:
                fpr, tpr, _ = roc_curve(self.data['Active'], self.data[i])
                roc_auc = round(auc(fpr, tpr),3)
                self.plot_roc(fpr, tpr, roc_auc, model = i, base_name = name)
            plt.savefig(f'./Image/{name}.png', dpi = 600)
            ...
            for i in self.simi_col:
                precision, recall, thresholds = precision_recall_curve(self.data[self.active_col], self.data[i])
                ap = round(average_precision_score(self.data[self.active_col], self.data[i]),3)
                self.plot_roc(precision, recall, ap, model = i, base_name = self.query.GetProp('_Name'))
